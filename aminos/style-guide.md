...
2. [运行时颜色函数](#2-运行时颜色函数-color)
3. [ansi 常量（表格数据着色）](#3-ansi-常量coloransicolor)
4. [installer 颜色](#4-installer-颜色)
5. [speedtest 颜色](#5-speedtest-颜色)
...
| 上下文 | 函数 | 当前颜色 | 代码文件 |
|--------|------|----------|----------|
| 错误标签 `"错误:"` | `color::red(...)` | 红色 | `main.rs` `print_clap_error()`, `run()` |
| 用法标签 `"用法:"` | `color::bold_yellow(...)` | **加粗黄色** | `main.rs` `print_clap_error()` |
| 灰色辅助提示 | `color::gray(...)` | 灰色 | `main.rs` `print_clap_error()` |
| 跳过提示 `"跳过"` | `color::yellow(...)` | 黄色 | `main.rs` `run_install`, `run_upgrade`, `run_uninstall` |

### 2.2 升级 (`run_upgrade`)

| 元素 | 函数 | 当前颜色 | 代码位置 |
|------|------|----------|----------|
| `"XXX 已是最新"` | `color::gray(...)` | 灰色 | `main.rs:402` |
| 当前版本（有更新时） | `color::yellow(...)` | 黄色 | `main.rs:409` |
| 源版本（有更新时） | `color::green(...)` | 绿色 | `main.rs:410` |
| `"升级 XXX 失败"` | `color::yellow(...)` | 黄色 | `main.rs:421` |
| 统计行（检查/升级结果） | `color::gray(...)` | 灰色 | `main.rs:430,434` |

### 2.3 列表 (`run_list`)

| 元素 | 函数/常量 | 当前颜色 | 代码位置 |
|------|----------|----------|----------|
| `"未找到源定义"` 提示 | `color::yellow(...)` | 黄色 | `main.rs:519` |
| 底部统计 `"共 N 项"` | `color::gray(...)` | 灰色 | `main.rs:657` |

### 2.4 信息 (`run_info`)

| 元素 | 函数 | 当前颜色 | 代码位置 |
|------|------|----------|----------|
| 软件显示名 | `color::green(...)` | 绿色 | `main.rs:670,701` |
| 版本号（带 `← 默认` 标记） | `color::green(...)` | 绿色 | `main.rs:773` |
| 已安装状态 `"已安装 (版本 X)"` | `color::green(...)` | 绿色 | `main.rs:721,729` |
| 未安装状态 `"未安装"` | `color::gray(...)` | 灰色 | `main.rs:747` |
| 标签 `"标识符:"` `"分类:"` `"官网:"` | `color::gray(...)` | 灰色 | `main.rs:710,715,716` |
| `"可用版本:"` | `color::gray(...)` | 灰色 | `main.rs:752` |
| `"类型:"` `"下载:"` | `color::gray(...)` | 灰色 | `main.rs:774,775` |
| 版本号（信息中） | `color::cyan(...)` | 青色 | `main.rs:688` |

### 2.6 source 命令 (`run_source` / `run_dirs` / `run_cache`)

#### `run_dirs`

| 元素 | 函数 | 当前颜色 | 代码位置 |
|------|------|----------|----------|
| `"aminos 数据目录一览"` | `color::bold_cyan(...)` | **加粗青色** | `main.rs:823` |
| 节标题（可执行/源定义/缓存/记录/快捷方式） | `color::bold_yellow(...)` | **加粗黄色** | `main.rs:825,829,833,837,841` |
| `"数据根目录"` | `color::bold_yellow(...)` | **加粗黄色** | `main.rs:845` |

#### `run_cache`

| 元素 | 函数 | 当前颜色 | 代码位置 |
|------|------|----------|----------|
| 清除成功 `"已清除 N 个缓存文件"` | `color::green(...)` | 绿色 | `main.rs:885` |
| 一致性标记 `" ⚠"`（不一致） | `color::yellow(...)` | 黄色 | `main.rs:924` |
| 一致性标记 `" ✓"`（一致） | `color::green(...)` | 绿色 | `main.rs:927` |
| `"下载缓存"` 节标题 | `color::bold_yellow(...)` | **加粗黄色** | `main.rs:944` |
| 缓存路径 | `color::gray(...)` | 灰色 | `main.rs:944` |
| 图例 `"✓ 版本与源定义一致 ⚠ 与源定义不一致"` | `color::green(...)` / `color::yellow(...)` | 绿/黄 | `main.rs:962` |
| 页脚统计 `"共 N 个文件"` | `color::gray(...)` | 灰色 | `main.rs:965,966,967` |

---

## 3. ansi 常量 (`color::ansi::*`)
...
| 状态 | 当前颜色 | 常量 | 代码位置 |
|------|----------|------|----------|
| `"下载中"` (黄色) | `\x1b[33m` | `color::ansi::YELLOW` | `main.rs:499` |
| `"已下载"` (青色) | `\x1b[36m` | `color::ansi::CYAN` | `main.rs:501` |

### 3.2 run_list 表格行

| 用途 | 当前颜色 | 常量 | 代码位置 |
|------|----------|------|----------|
| 有源定义 | 绿色 | `color::ansi::GREEN` | `main.rs:541,565,583,587` |
| 无源定义 / 未安装 / 未下载 | 灰色 | `color::ansi::GRAY` | `main.rs:541,545,547,550,580,587` |
| 行末复位 | 复位 | `color::ansi::RESET` | `main.rs:646,650` |
...
## 5. speedtest 颜色（`as source speedtest`）
